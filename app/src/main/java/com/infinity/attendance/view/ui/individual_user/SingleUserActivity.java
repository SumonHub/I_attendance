...
public class SingleUserActivity extends AppCompatActivity implements View.OnClickListener, OnDataUpdateListener {
    ...
    private boolean isHoliday() {
        ...
        dataViewModel.checkIsAnyOffday(selectedUser.getApi_key()).observe(this, new Observer<ApiResponse>() {
            @Override
    ...
    private void _getData(OnApiResponse<User> onApiResponse) {
        ...
        dataViewModel.getUserByKeyLiveData(selectedUser.getApi_key()).observe(this, new Observer<ApiResponse<User>>() {
            @Override
            public void onChanged(ApiResponse<User> userApiResponse) {
                if (userApiResponse != null && !userApiResponse.isError()) {
                    onApiResponse.onSucceed(userApiResponse.getResults().get(0));
                } else {
            ...
            public void onShow(DialogInterface dialogInterface) {
                ...
                btnPositive.setOnClickListener(new View.OnClickListener() {
                    ...
                    public void onClick(View view) {
                        ...
                        _updatePassword(superUser.getApi_key(), inputCnfPass.getText().toString(),
    ...
    private void _updatePassword(String api_key, String string, OnApiResponse<Boolean> onApiResponse) {
        DataViewModel dataViewModel = new DataViewModel();
        dataViewModel.resetPassword(api_key, string).observe(this, new Observer<ApiResponse>() {
            @Override
            public void onChanged(ApiResponse apiResponse) {
                if (apiResponse != null && !apiResponse.isError()) {
    ...
    private void addAttendance(String address, int statusCode) {
        ...
        DataViewModel dataViewModel = new DataViewModel();
        dataViewModel.addAttendance(selectedUser.getApi_key(),
                selectedUser.getUid(),
                currentDate,
                currentTime,
                address,
            ...
            public void onChanged(ApiResponse<Attendance> attendanceApiResponse) {
                if (attendanceApiResponse != null && !attendanceApiResponse.isError()) {
                    updateTodayAttendanceUI(attendanceApiResponse.getResults().get(0));
                } else {
